...
// دالة لإنشاء النجوم بناءً على تقييم من 10 (تعرض 10 نجوم كحد أقصى)
// دالة لإنشاء النجوم بناءً على تقييم من 10 (تعرض 10 نجوم كحد أقصى)
function create_php_rating_stars($rating) {
    ...
    for ($i = 1; $i <= 10; $i++) {
        if ($r >= $i) {
            $html .= '<i class="fas fa-star"></i>'; // نجمة كاملة
        } elseif ($r >= $i - 0.5) {
            $html .= '<i class="fas fa-star-half-alt"></i>'; // نصف نجمة
        } else {
            $html .= '<i class="far fa-star"></i>'; // نجمة فارغة
        }
    }
    return $html;
}
                        ...
                        <div class="related-games-grid" id="relatedGamesGrid">
                            <?php foreach ($relatedGames as $game): ?>
                                <a href="download.php?id=<?php echo $game['id']; ?>&title=<?php echo format_title_for_url($game['title']); ?>" class="related-game-card">
                                    ...
                                    <h5 class="related-game-title"><?php echo htmlspecialchars($game['title']); ?></h5>
                                </a>
                            <?php endforeach; ?>
                        </div>
                    </section>
                    ...
                    <div class="sidebar-card">
                        ...
                        <div class="popular-games-section">
                            <div class="popular-games-list" id="popularGamesList">
                                ...
                                ?>
                                    <!-- إضافة الكلاسات والأيقونات المطلوبة -->
                                    <div class="popular-game-item" onclick="window.location.href='download.php?id=<?php echo $game['id']; ?>&title=<?php echo $formattedTitle; ?>'">
                                        <div class="popular-game-image">
                                            <img src="<?php echo get_image_path($game['image']); ?>" alt="<?php echo htmlspecialchars($game['title']); ?>" class="popular-game-actual-image loaded">
                                        </div>
                                        <div class="popular-game-info">
                                            ...
                                            <div class="popular-game-meta">
                                                ...
                                                <div class="popular-game-rating">
                                                    <?php 
                                                    // دالة توليد النجوم البسيطة للـ PHP
                                                    $rating = isset($game['rating']) ? floatval($game['rating']) : 0;
                                                    $fullStars = floor($rating);
                                                    for($i = 0; $i < 5; $i++) {
                                                        if($i < $fullStars) echo '<i class="fas fa-star"></i>';
                                                        else echo '<i class="far fa-star"></i>'; // نجمة فارغة
                                                    }
                                                    ?>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="popular-game-trend up">
                                            <i class="fas fa-arrow-up"></i>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
