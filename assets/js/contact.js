...
e.stopPropagation();
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            // Form will submit normally to the server for PHP processing.
        });
    }
    ...
    function setupSearch() {
        ...
        // ✅ التعديل هنا: قراءة البيانات من allGamesData أو gamesData
    ...
    const animatedElements = document.querySelectorAll(
        '.about-section, .feature-card, .contact-info, .contact-form, .privacy-content h2, .privacy-content h3'
    );
...
window.addEventListener('error', function(e) {
    ...
    document.querySelectorAll('.category-dropdown-item').forEach(item => {
        item.addEventListener('click', function(e) {
            ...
            if (categoryId) {
                window.location.href = `categories.php?category=${categoryId}`;
            }
        });
    });
});